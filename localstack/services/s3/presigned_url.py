...
from localstack.http.request import Headers
...
def s3_presigned_url_request_handler(_: HandlerChain, context: RequestContext, __: Response):
    ...
    is_v2 = all(p in context.request.args for p in SIGNATURE_V2_PARAMS)
    is_v4 = all(p in context.request.args for p in SIGNATURE_V4_PARAMS)

    if not is_v2 and any(p in context.request.args for p in SIGNATURE_V2_PARAMS):
        LOGGER.info("Presign signature calculation failed")
    ...
    if not is_v4 and any(p in context.request.args for p in SIGNATURE_V4_PARAMS):
        LOGGER.info("Presign signature calculation failed")
...
def _prepare_request_for_sig_v4_signature(
    ...
    :return: Request
    """
    request_headers = copy.deepcopy(context.request.headers)
