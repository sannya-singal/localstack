"""
I would like to spawn only one ThreadPoolExecutor for one operation call
-> Not spawn an executor for each type of notification
The goal would be that they would all share one Executor?
...
def _matching_filter(
    ...
    # TODO: for now only matching on key name?
    ...
    def _get_event_payload(
        ...
        record = EventRecord(
            ...
            s3=S3NotificationContent(
                ...
                object={
                    "key": ctx.key_name,  # todo check quote?? depends usage
        ...
        # todo check this, logic seems duplicated
...
class EventBridgeNotifier(BaseNotifier):
    ...
    @staticmethod
    def should_notify(ctx: S3EventNotificationContext, config: Dict) -> bool:
        # TODO: EventBridge filter/rules? everything??
        return True

    def validate_configuration_for_notifier(
        self, configurations: List[Dict], skip_destination_validation=False
    ):
        # TODO: is there validation for EventBridge?
        return
